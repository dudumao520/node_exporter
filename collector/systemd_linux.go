...
func (c *systemdCollector) Update(ch chan<- prometheus.Metric) error {
	...
	begin = time.Now()
	err = c.collectUnitStatusMetrics(ch, units)
	if err != nil {
		return fmt.Errorf("couldn't get unit status metrics: %s", err)
	}
	log.Debugf("systemd collectUnitStatusMetrics took %f", time.Since(begin).Seconds())

	begin = time.Now()
	err = c.collectUnitStartTimeMetrics(ch, units)
	if err != nil {
		return fmt.Errorf("couldn't get unit start time metrics: %s", err)
	}
	log.Debugf("systemd collectUnitStartTimeMetrics took %f", time.Since(begin).Seconds())

	begin = time.Now()
	err = c.collectUnitTasksMetrics(ch, units)
	if err != nil {
		return fmt.Errorf("couldn't get unit tasks metrics: %s", err)
	}
	...
	begin = time.Now()
	err = c.collectSockets(ch, units)
	if err != nil {
		return fmt.Errorf("couldn't get unit socket metrics: %s", err)
	}
...
func (c *systemdCollector) collectUnitStatusMetrics(ch chan<- prometheus.Metric, units []unit) error {
	conn, err := c.newDbus()
	if err != nil {
...
func (c *systemdCollector) collectSockets(ch chan<- prometheus.Metric, units []unit) error {
	...
	for _, unit := range units {
		...
		acceptedConnections = acceptedConnectionCount.Value.Value().(uint32)
		...
		currentConnections = currentConnectionCount.Value.Value().(uint32)
		...
		if err != nil {
			...
			refusedConnections = &nRefused
		}

		ch <- prometheus.MustNewConstMetric(
			c.socketAcceptedConnectionsDesc, prometheus.CounterValue,
			float64(acceptedConnections), unit.Name)
		ch <- prometheus.MustNewConstMetric(
			c.socketCurrentConnectionsDesc, prometheus.GaugeValue,
			float64(currentConnections), unit.Name)
		if refusedConnections != nil {
			ch <- prometheus.MustNewConstMetric(
				c.socketRefusedConnectionsDesc, prometheus.GaugeValue,
				float64(*refusedConnections), unit.Name)
		}
	}
	return nil
}

func (c *systemdCollector) collectUnitStartTimeMetrics(ch chan<- prometheus.Metric, units []unit) error {
	conn, err := c.newDbus()
	if err != nil {
		return fmt.Errorf("couldn't get dbus connection: %s", err)
	}
...
func (c *systemdCollector) collectUnitTasksMetrics(ch chan<- prometheus.Metric, units []unit) error {
	conn, err := c.newDbus()
	if err != nil {
		return fmt.Errorf("couldn't get dbus connection: %s", err)
	}
